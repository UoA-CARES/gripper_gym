...
class FourFingerRotation(FourFingerTask):
    ...
    def _environment_info_to_state(self, environment_info):
        ...
        # # cube from origin
        # for i in range(0,3):
        #     state += [round(environment_info["cube_depth"][i],2)]

        # grippers from origin
        # for i in range(0,12):
        #     state += [round(environment_info["gripper_depths"][i],2)]
        ...
        if self.touch_config == True:
            if self.tactile_server.error == True:
                ...
                time.sleep(10)
                self.tactile_server = server.Server(port="/dev/Arduino_Flat", baudrate=921600, socket_port=self.socket_port)
                self.server_thread = threading.Thread(target=self.tactile_server.start)
                self.server_thread.daemon = True
                self.server_thread.start()
                time.sleep(10)
...
class FourFingerRotationFlat(FourFingerRotation):
    ...
    def _reward_function(self, previous_environment_info, current_environment_info):
        ...
        # if reward < -100:
        #     reward = -100
        # # No reward for not moving the cube
        # if abs(delta) < 1:
        #     reward = 0
        #####
        ...
        if delta_reward < -100:
            ...
            delta_reward = 0
        # if delta_reward < 0:        # If reward is negative set flag to ignore touch sensor reward
        ...
        #Touch-based reward
        if self.touch_config == True:# and touch_included == True:
            if reward != 0:
